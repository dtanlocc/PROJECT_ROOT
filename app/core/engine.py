...
class ProEngine:
    ...
    # --- HÀM TÌM FILE DÙNG TÊN SAFE_STEM (Đồng bộ với GUI) ---
    def get_existing_output(self, step: int, safe_stem: str):
        """Tìm file dựa trên safe_stem (ID băm) để khớp chính xác với video đang xử lý."""
    ...
    def process_one(self, video_path: Path):
        ...
        try:
            ...
            if getattr(self.cfg.pipeline, "run_s1", True):
                ...
                wav = self.get_existing_output(1, safe_stem)
            ...
            if getattr(self.cfg.pipeline, "run_s2", True):
                ...
                sep_dir = self.get_existing_output(2, safe_stem)
            ...
            if getattr(self.cfg.pipeline, "run_s3", True):
                ...
                srt_raw = self.get_existing_output(3, safe_stem)
            ...
            if getattr(self.cfg.pipeline, "run_s4", True):
                ...
                srt_trans = self.get_existing_output(4, safe_stem)

            # BÁO CÁO HOÀN TẤT B4 ĐỂ GUI TỰ NẠP SUB VÀ DỪNG LUỒNG
            self._report_progress("B4 Hoàn tất", safe_stem, force_step=4)
            ...
            if getattr(self.cfg.pipeline, "run_s5", True):
                ...
                vid_sub = self.get_existing_output(5, safe_stem) or safe_video_path
            ...
            if getattr(self.cfg.pipeline, "run_s6", True):
                ...
                logger.info(f"⏭️ Hoàn thành các bước đã chọn cho: {original_stem}")
    ...
    def _report_progress(self, step_name=None, safe_stem=None, done=False, force_step=None):
        if not self._on_progress_cb: return
        with self._progress_lock:
            if done and safe_stem: 
            ...
            # Nếu có force_step (như 4), GUI sẽ nhận biết để dừng luồng
            val = force_step if force_step is not None else self._progress_completed
            self._on_progress_cb(val, self._progress_total, cur)
    ...
    def _report_step_ratio(self, step_name, safe_stem, ratio):
        ...
        with self._progress_lock:
            ...
            cur = list(self._progress_current.values())
            # Trả về số nguyên để tránh lỗi Overflow trên GUI
